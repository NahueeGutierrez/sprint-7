...
def pagina_de_exito(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            mensaje = '¡Operación Exitosa! Tu operación ha sido completada con éxito. Por favor, verifica la base de datos itbank.db.'
    
    else:
        form = CrearPrestamoForm()

    return render(request, 'pagina_de_exito.html', {'mensaje': mensaje, 'form': form})